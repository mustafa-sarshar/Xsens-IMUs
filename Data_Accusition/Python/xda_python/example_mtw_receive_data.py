import sys
import xsensdeviceapi.xsensdeviceapi_py37_64 as xda # import xsensdeviceapi as xda doesn't work
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in range(portInfoArray.size()):
            if portInfoArray[i].deviceId().isAwinda2() or portInfoArray[i].deviceId().isMtig():
                mtPort = portInfoArray[i]
                break

        if mtPort.empty():
            raise RuntimeError("No connected device found. Aborting.")
        ...
        if not device.enableRadio(15):
            raise RuntimeError("could not enable radio")
